...
Dictionary<int, int> fillDictionary(int[,] matrix)
{
  ...
  for (int i = 0; i < matrix.GetLength(0); i++)
  {
    ...
    {
      if (!dict.ContainsKey(matrix[i, j])) dict.Add(matrix[i, j], 1);
      else
      {
        dict[matrix[i, j]]++;
      }
...
void Printdict(Dictionary<int, int> dictionary)
{
  ...
  {
    Console.WriteLine($"Число{VARIABLE.Key} содержиться {VARIABLE.Value} раз");
  }
}
freqDict = fillDictionary(matrix);
Printdict(freqDict);